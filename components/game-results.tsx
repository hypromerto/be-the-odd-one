...
export default function GameResults({ players, themes, roomId, isHost }: GameResultsProps) {
    ...
    return (
        <div className="flex flex-col items-center space-y-6 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-5xl mx-auto text-indigo-800">
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
                {sortedPlayers.slice(1).map((player, index) => (
                    <Card key={player.id} className="bg-white/80 rounded-lg shadow-md p-4 flex items-center justify-between">
                        ...
                        <div className="text-xl font-bold text-purple-700 ml-2">
                            #{index + 2}
                        </div>
                    </Card>
                ))}
                        ...
                        <CardContent>
                            <ul className="space-y-2">
                                {theme.answers.map((answer, i) => (
                                    <li key={i} className="text-sm sm:text-base flex justify-between items-center py-1 border-b border-gray-100 last:border-b-0">
