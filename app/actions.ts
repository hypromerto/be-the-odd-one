...
export async function submitThemes(roomId: string, themes: string[]) {
  ...
  const { data: playerUpdate, error: playerUpdateError } = await supabase
      .from("players")
      .update({ theme_ready: true })
      .eq("id", currentPlayer.id)
      .select()
      .single()

  if (playerUpdateError) {
    console.error("Error updating player theme_ready status:", playerUpdateError)
  ...
  const { count: readyPlayerCount, error: readyPlayerCountError } = await supabase
      .from("players")
      .select("id", { count: "exact" })
      .eq("room_id", roomId)
      .eq("theme_ready", true)

  if (playerCountError || themeCountError || readyPlayerCountError) {
    console.error(
        "Error checking theme submission status:",
        playerCountError || themeCountError || readyPlayerCountError,
    )
  } else if (readyPlayerCount === playerCount) {
    // All players have submitted themes
    await supabase.channel(`room:${roomId}`).send({
      type: "broadcast",
  ...
  await supabase.channel(`room:${roomId}`).send({
    ...
    payload: { themes: data, playerId: currentPlayer.id },
  })
