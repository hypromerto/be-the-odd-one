...
export default function AnswerInput({ roomId, theme, currentPlayer }: AnswerInputProps) {
  ...
  useEffect(() => {
    if (gameState?.is_timed_mode && gameState?.timer_started && !isSubmitted) {
      toast({
        title: t("playerAnswered", { playerName: gameState?.players.find((p) => p.id === gameState?.first_submit_player_id)?.name }),
  ...
  }, [gameState?.is_timed_mode, gameState?.timer_started, isSubmitted, t, toast])
